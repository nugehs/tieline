...
/** Load and resolve seam.config.json. Repo paths are resolved relative to the config file. */
...
function findConfig() {
  let dir = process.cwd();
  for (;;) {
    const p = path.join(dir, 'seam.config.json');
    if (fs.existsSync(p)) return p;
    const parent = path.dirname(dir);
    if (parent === dir) break;
    dir = parent;
  }
  throw new Error('No seam.config.json found (searched up from cwd). Pass --config <path>.');
}
